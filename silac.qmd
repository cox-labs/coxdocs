...
[Load → Generic matrix upload](docs/genericmatrixupload.html) is denoted by the green arrow on the top left corner of the Perseus window or load the file using the drag and drop function of Perseus.
...
*Comment: After each applied operation a new matrix will be generated. Every matrix that you are creating can be saved with [Export → Generic matrix export](docs/tabseparatedexport.html) and re-imported later with [Load → Generic matrix upload](docs/genericmatrixupload.html).*

## Filtering

After loading the matrix, we filter out the reverse proteins and the proteins that are only identified by site.
This is done by using the function [Processing → Filter rows → Filter rows based on categorical column](docs/filtercategoricalcolumn.html), because both the "Reverse" column and the "Only identified by site" column are categorical.
...
Therefore we use [Processing → Filter rows → Filter rows based on valid values](docs/filtervalidvaluesrows.html).
...
Then the ratios will be logarithmized by using the function [Processing → Basic → Transform](docs/transformationproc.html) again.
...
So we rename the main columns with the help of regular expressions and the function [Processing → Rearrange → Rename columns \[Reg.ex.\]](docs/renamecolumnsregexp.html).
...
In [Processing → Annot.columns → Add annotation](docs/addannotationtomatrix.html) are several options for adding annotations to your data.
...
Therefore open a scatter plot with [Analysis → Visualization → Scatter plot](docs/scatterplotanalysis.html).
...
Estimate the density for two columns with [Processing → Basic → Density estimation](docs/densityestimationprocessing.html).
...
Open a multi scatter plot using [Analysis → Visualization → Multi scatter plot](docs/multiscatteranalysis.html).
...
Open a histogram using [Analysis → Visualization → Histogram](docs/histogram.html).
...
For instance try normalizing the data by subtracting the median from each distribution with [Processing → Normalization → Subtract](docs/subtract.html) and check what changed by looking at the resulting histograms.
...
Open a profile plot with [Analysis → Visualization → Profile plot](docs/profileplot.html).
...
Open a 3D scatter plot with [Analysis → Visualization → 3D plot](docs/scatterplot3danalysis.html).
...
Go to [Processing → Annot.rows → Categorical annotation rows](docs/createcategoricalannotrow.html).
...
If you export the data now using x[Export → Generic matrix export](docs/tabseparatedexport.html) and re-import it later with [Load → Generic matrix upload](docs/genericmatrixupload.html), the grouping is still defined.
...
Since we are here only interested in absence/presence in a certain cell line and not so much in single replicates, we first average ratios within groups using [Processing → Annot.rows → Average groups](docs/averagegroups.html).
...
Then when performing [Analysis → Misc.→ Numeric venn diagram](docs/numericvenndiagram.html) you will get some statistical information about the occurrence of proteins in the different cell lines.
...
To find out which proteins are differentially regulated between the cell lines go back to the matrix before "averaging within groups" and apply [Processing → Tests → Multiple-samples tests](docs/multiplesampletestprocessing.html).
...
For this we can use the Fisher exact test in [Processing → Annot.columns → Fisher exact test](docs/fisherexacttestproces.html).
...
Use [Processing → Normalization → Z-score](docs/zscore.html).
...
Now perform hierarchical clustering of rows and columns using [Analysis → Clustering/PCA → Hierarchical clustering](docs/hierarchicalcluster.html).
...
We filter for valid values using [Processing → Filter rows → Filter rows based on valid values](docs/filtervalidvaluesrows.html) such that every row contains nine valid values.
...
Then perform PCA with [Analysis → Clustering/PCA → Principal component analysis](docs/principalcomponentanalysis.html) and explore the results.
