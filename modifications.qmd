...
Load the file "Phospho (STY)Sites.txt" from the "combined/txt" folder of the MaxQuant output. [Load → Generic matrix upload](docs/genericmatrixupload.html) is denoted by the green arrow on the top left corner of the Perseus window or load the file using the drag and drop function of Perseus.
...
*Comment:*
_After each applied operation a new matrix will be generated. Every matrix that you are creating can be saved with [Export → Generic matrix export](docs/tabseparatedexport.html) and re-imported later with [Load → Generic matrix upload](docs/genericmatrixupload.html)._

# Filtering

After loading the matrix, we filter out the reverse (decoy) database hits, the contaminants and the proteins with a localization probability $<0.75$. Both the "Reverse" column and the "Contaminant" column are categorical. Therefore we use [Processing → Filter rows → Filter rows based on categorical column](docs/filtercategoricalcolumn.html).
...
Then we filter the site table to retain only these sites that have a localization probability $>0.75$. Therefore we use [Processing → Filter rows → Filter rows based on numerical/main column](docs/filternumericalcolumn.html).
...
Now we proceed with the standard data processing steps and take the logarithm of values in the main columns with [Processing → Basic → Transform](docs/transformationproc.html).
...
Next, we are doing some cosmetics by shortening the names of the main columns and remove the prefix "Intensity" with [Processing → Rearrange → Rename columns \[reg. ex.\]](docs/renamecolumnsregexp.html).
...
If you want to rename the columns manually without the help of regular expressions you can use [Processing → Rearrange → Rename columns](docs/renamecolumns.html).
...
Now we add protein annotations with [Processing → Annot. columns → Add annotation](docs/addannotationtomatrix.html). There are several options for adding annotations to your data. You can select from any annotation file that is present in the "Perseus/conf/annotations/" folder.
...
Third, we add known sites with [Processing → Modifications → Add known sites](docs/addknownsites.html). We are interested in phosphorylation and define them as "Modification". Also we have to define the column, which contains the Uniprot identifier ("Uniprot column") and the column containing the sequence window.
...
And last but not least, we add known kinase-substrate relations with the function [Processing → Modifications → Kinase-substrate relations](docs/kinasesubstraterelations.html). For that we specify the column containing the Uniprot identifier ("Uniprot column") and the one containing the sequence window.
...
Define a sample grouping according to C, E15 and N using [Processing → Annotation rows → Categorical annotation rows](docs/createcategoricalannotrow.html).
...
Next, rows without any valid values in the main columns won't be particularly useful for quantitative analysis, therefore we can remove them by filtering [Processing → Filter rows → Filter rows based on valid values](docs/filtervalidvaluesrows.html).
...
Now let's look at some histograms of the data. Use [Analysis → Visualization → Histogram](docs/histogram.html).
...
Nevertheless, for demonstration, you could subtract the median of each column with [Processing → Normalization → Subtract](docs/subtract.html).
...
Now open a scatter plot or a multi scatter plot to see, if you are happy with the correlation between replicates. Here we use a multi scatter plot [Analysis → Visualization → Multi scatter plot](docs/multiscatteranalysis.html), because it gives us a great overview in one figure. Also correlation coefficients can be calculated for each plot. Before you do this remind yourself that this is label free data, which may calibrate your expectations to low values.
...
It is recommended to impute missing values after filtering, thus we don't have any missing values in the data for further analyses. Therefore use [Processing → Imputation → Replace missing values from normal distribution](docs/replacemissingfromgaussian.html).
...
Now check which annotation terms are enriched in the changing sites. For this we use a Fisher exact test [Processing → Annot. columns → Fisher exact test](docs/fisherexacttestproces.html).
...
Second we are performing Z-scoring of rows with [Processing → Normalization → Z-score](docs/zscore.html).
...
And finally we use hierarchical clustering [Analysis → Clustering/PCA → Hierarchical clustering](docs/hierarchicalcluster.html) to visualize the cluster of the data.
...
There are many more ways to further explore the data. If you have time try e.g. [Analysis → Clustering/PCA → Principal component analysis](docs/principalcomponentanalysis.html). Note that principal component analysis is best done without prior z-scoring of the data.
